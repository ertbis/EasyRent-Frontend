...
const ErrorModal :FC<ErrorProps>  = ({setLoginModal, text}) => {
    useEffect(() => {
        AOS.init();
        AOS.refresh();
      }, []);
    ...
    <div data-aos="zoom-in"  className="relative w-80 mx-4 h-[10rem]  rounded-lg shadow bg-gray-100">
        <div  className="z-[1500]  " onClick={() => setLoginModal(false)}>
        <FaTimes onClick={() => setLoginModal(false)}  size={30} className=" text-black absolute top-3 right-2.5" />
        </div>
